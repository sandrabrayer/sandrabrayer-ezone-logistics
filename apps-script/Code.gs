...
// ===== Exceptional-events register (אירועים חריגים) — Apps-Script wiring around the MIRROR:events block =====

// The allowed event categories, from the Config event_types spec (fallback אחר only, logged).
function eventTypesList_() {
  return parseEventTypes(getConfig('event_types'), function (m) { Logger.log(m); });
}

// Update specific Events cells by header name (id lookup). Used by the edit/close handler.
function updateEventRow_(id, fields) {
  ...
  for (var r = 1; r < data.length; r++) {
    if (String(data[r][idCol]).replace(/^\s+|\s+$/g, '') === String(id)) {
      for (var key in fields) {
...
// Report (create) an exceptional event. Role + house-scope gated HERE (independent of the gateway):
// maintenance is refused; a coordinator may only report for their OWN house. created_by is the token
// identity — NEVER the client body. Every create is audited.
function handleCreateEvent_(p, actor) {
  // The client submits the house NAME (like createRequest); the register stores the CANONICAL id.
  var houseName = String((p && p.house) == null ? '' : p.house).replace(/^\s+|\s+$/g, '');
  var houseId = DIGEST_HOUSE_IDS_[houseName] || '';
  if (!houseId) return jsonOut_({ ok: false, error: 'בית לא חוקי' });
  if (!eventCreatePermitted(actor.role, actor.scope, houseName, getHouseCluster_(houseName))) return forbidden_();
  var err = eventCreateError(p, eventTypesList_());
  if (err) return jsonOut_({ ok: false, error: err });
  var id = genId_('EVT');
  var normalized = {
    house: houseId, occurred_at: p.occurred_at, event_type: p.event_type, severity: p.severity,
    description: p.description, immediate_action: p.immediate_action, notes: p.notes
  };
  var row = buildEventRow(normalized, actor, id, new Date().toISOString());
  appendRow_('Events', row);
  writeAuditEntry(id, '', 'פתוח', actor.name, 'אירוע חריג נרשם (' + row.event_type + '/' + row.severity + ')');
...
function handleUpdateEvent_(p, actor) {
  ...
  var ev = null;
  for (var i = 0; i < events.length; i++) {
    if (String(events[i].id) === id) { ev = events[i]; break; }
  }
  if (!ev) return jsonOut_({ ok: false, error: 'Event not found' });

  var merged = {
    root_cause: p.root_cause != null ? String(p.root_cause) : (ev.root_cause || ''),
    lessons: p.lessons != null ? String(p.lessons) : (ev.lessons || ''),
    corrective_request_id: p.corrective_request_id != null ? String(p.corrective_request_id) : (ev.corrective_request_id || ''),
    status: p.status != null ? String(p.status) : (ev.status || 'פתוח'),
    notes: p.notes != null ? String(p.notes) : (ev.notes || '')
  };
  var corr = String(merged.corrective_request_id).replace(/^\s+|\s+$/g, '');
  var correctiveExists = corr === '' ? true : !!getRequestById(corr);
  var err = eventUpdateError(merged, correctiveExists);
  if (err) return jsonOut_({ ok: false, error: err });

  var wasClosed = String(ev.status == null ? '' : ev.status).replace(/^\s+|\s+$/g, '') === 'נסגר';
  var nowClosed = merged.status === 'נסגר';
  var toWrite = {
    root_cause: merged.root_cause, lessons: merged.lessons,
    corrective_request_id: corr, status: merged.status, notes: merged.notes
  };
  if (nowClosed && !wasClosed) toWrite.closed_at = new Date().toISOString();
  if (!nowClosed && wasClosed) toWrite.closed_at = '';
  updateEventRow_(id, toWrite);
  writeAuditEntry(id, ev.status || '', merged.status, actor.name, (nowClosed && !wasClosed) ? 'אירוע נסגר' : 'אירוע עודכן');
  return jsonOut_({ ok: true, id: id });
}

// Exceptional-events analysis panel for /management (canManage-gated). Derived, read-only; writes nothing.
function readEventsAnalysis_() {
  var maps = { idToName: canonicalHouseIdToName_() };
  return eventsAnalysis(readObjects_('Events'), maps, openHouseIds_(), todayYmd_(), function (m) { Logger.log(m); });
}
